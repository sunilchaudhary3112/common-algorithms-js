...
*/
module.exports = exports = function(input, pattern, alphabetSize) {
  if (typeof input !== 'string') return 'Boyer-Moore-Horspool String Match: The input must be a string.';
  if (typeof pattern !== 'string') return 'Boyer-Moore-Horspool String Match: The pattern must be a string.';
  if (typeof alphabetSize !== 'number') return 'Boyer-Moore-Horspool String Match: The alphabet size must be a number.';
  ...
  while (k < input.length) {
    var j = pattern.length - 1, i = k;
    while (j >= 0 && input[i] === pattern[j]) j -= 1, i -= 1;
    if (j === -1) return i + 1;
